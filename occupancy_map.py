...
class OccupancyMap:
    ...
    def _init_map_from_world(self):
        ...
        voxel_dimensions_metric = [] # list of x,y,z
        voxel_dimensions_indices = [] # metric/resolution ?
        for i in range(3):
        ...
        self.map = np.zeros(voxel_dimensions_indices, dtype=bool) # (x,y,z) array
        self.origin = np.array(bounds[0::2]) # (3,) array
        ...
        self.neighbor_dist = np.zeros((3,3,3))
        for i in range(-1, 2):
            for j in range(-1, 2):
                for k in range(-1, 2):
                    if i == 0 and j == 0 and k == 0:
                        continue
                    self.neighbor_dist[i+1, j+1, k+1] = np.linalg.norm(np.array([i, j, k])*self.resolution)
    ...
    def euclidean_distance(self, voxel_index1, voxel_index2):
        """
        Return the Euclidean distance between two voxel indices.
        """
        return np.linalg.norm(self.index_to_metric_center(voxel_index1) - self.index_to_metric_center(voxel_index2))

    def get_neighbors(self, voxel_index):
        """
        Return a list of valid neighboring voxel indices.
        """
        neighbors = []
        for i in range(-1, 2):
            for j in range(-1, 2):
                for k in range(-1, 2):
                    if i == 0 and j == 0 and k == 0:
                        continue
                    neighbor = (voxel_index[0]+i, voxel_index[1]+j, voxel_index[2]+k)
                    if self.is_valid_index(neighbor):
                        dist = self.neighbor_dist[i+1, j+1, k+1]
...
if __name__ == "__main__":
    ...
    # some tests
    print(oc.is_valid_index((0,0,0)))
    print(oc.is_valid_index((100,100,100)))
    print(oc.is_valid_index((-1,-1,-1)))
    print(oc.is_valid_metric(np.array([0,0,0])))
    print(oc.is_valid_metric(np.array([100,100,100])))
    print(oc.is_valid_metric(np.array([-1,-1,-1])))
    print(oc.is_valid_metric(np.array([1,1,1])))
    print(oc.is_valid_metric(np.array([.1,.1,.1])))
    print(oc.is_valid_metric(np.array([5.0,5.0,5.0])))
    
    # also some tests
    print(oc.index_to_metric_negative_corner((0,1,49)))
    print(oc.index_to_metric_negative_corner((48,49,49)))
    print(oc.index_to_metric_center((0,1,0)))
    print(oc.index_to_metric_center((48,49,49)))
    print(oc.metric_to_index(np.array([0,0,0])))
    print(oc.metric_to_index(np.array([.1,4.9,2.5])))
    print(oc.metric_to_index(np.array([5.0,5.0,5.0])))
    print(oc.metric_to_index(np.array([-1,-1,-1])))
